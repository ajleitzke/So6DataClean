...
def prepare(dataFile, addressList):
    ...
    df_calls['Incident Address'] = df_calls['Incident Address'].apply(str.upper)
...
def export(df_joined):
    df_joined.to_excel('files/cleaned_data.xlsx', index=False)  # Export to excel
    return


if __name__ == '__main__':
    input('The following tool will clean South of 6 Call for Service data. Press ENTER to continue.\n')
    dataFile = 'files/' + input('What is the name of the updated file (include the .xlsx extension)? ')
